...
WHERE t.name = '物理 必修 第一册'
  ...
  AND s.number = 1
  AND s.is_deleted = 0
  AND NOT EXISTS (SELECT 1 FROM textbook_slide_deck d WHERE d.section_id = s.id AND d.is_deleted = 0)
LIMIT 1;
