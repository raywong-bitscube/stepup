...
func registerAPIRoutes(mux *http.ServeMux, cfg config.Config, db *sqlx.DB) {
	...
	mux.HandleFunc("POST /api/v1/admin/exam-source/import-records/upload-analyze", middleware.RequireAdminAuth(adminAuthService, adminExamSourceHandler.CreateImportRecordByAnalyze))
	mux.HandleFunc("GET /api/v1/admin/exam-source/import-records", middleware.RequireAdminAuth(adminAuthService, adminExamSourceHandler.ListImportRecords))
	mux.HandleFunc("GET /api/v1/admin/exam-source/import-records/{recordId}", middleware.RequireAdminAuth(adminAuthService, adminExamSourceHandler.GetImportRecord))
	mux.HandleFunc("POST /api/v1/admin/exam-source/import-records/{recordId}/create-paper", middleware.RequireAdminAuth(adminAuthService, adminExamSourceHandler.CreatePaperFromImportRecord))
