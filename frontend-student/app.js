(function () {
    ...
    if (!state.token) {
      root.innerHTML = `
        <div class="wrap">
          <div class="card">
            <h1>StepUp 学生端</h1>
            <p class="muted">手机号或邮箱注册/登录，上传试卷查看 AI 分析与改进计划。</p>
            <p class="muted" style="margin-top:8px">API：<strong>${escapeHtml(apiBase())}</strong>
              （可用 <code>?api=http://host:8080</code> 或 localStorage <code>stepup_api_base</code> 覆盖）</p>
            ${flash}
            <div class="tabs">
    ...
    root.innerHTML = `
      <div class="wrap">
        <div class="card toolbar">
          <div><h1 style="margin:0;font-size:20px">StepUp 学习工作台</h1>
          <p class="muted" style="margin:4px 0 0">上传试卷 · 查看分析 · 改进计划</p></div>
          <button type="button" class="btn secondary" id="btnOut">退出</button>
        </div>
        ${flash}
        <div class="card" id="uploadCard"></div>
        <div class="card"><h2>我的试卷</h2><p class="muted">点击一条查看分析与计划。</p><div id="paperPane"></div></div>
      </div>`;
    bindMain(root);
    loadCatalog()
      .then(() => {
      ...
      .catch((e) => {
        state.flash = { kind: 'err', msg: e.message || String(e) };
        mount(root);
      });
  }
  ...
  function renderAuthBody(host) {
    if (state.authTab === 'login') {
      ...
      host.querySelector('#bLogin').addEventListener('click', async () => {
        try {
    ...
    host.innerHTML = `
      <p class="muted">① 获取验证码 → ② 校验 → ③ 设置密码（开发环境接口会返回验证码）。</p>
      <div class="row">
      ...
      <div class="row">
        <div><label>设置密码</label><input id="rpw" type="password" /></div>
    ...
    host.querySelector('#bSet').addEventListener('click', async () => {
      try {
        ...
        state.flash = { kind: 'ok', msg: '注册成功，请切换到登录。' };
        state.authTab = 'login';
        mount(document.getElementById('app'));
      } catch (e) {
  ...
  function bindUpload(root) {
    root.querySelector('#bUp').addEventListener('click', async () => {
      ...
      try {
        await api('/api/v1/student/papers', { method: 'POST', form: fd });
        state.flash = { kind: 'ok', msg: '上传成功，已触发分析。' };
        mount(document.getElementById('app'));
      } catch (e) {
    ...
    const rows = state.papers
      .map(
        (p) =>
          ...
          <span class="muted" style="font-size:12px">${escapeHtml(p.file_name)} · ${escapeHtml(
            p.created_at || ''
          )}</span></div>
      ...
      .join('');
    pane.innerHTML = `<div class="paper-list">${rows || '<p class="muted">暂无试卷</p>'}</div><div id="detail"></div>`;
  ...
  async function showDetail(root) {
    ...
    try {
      ...
      pane.innerHTML = `
        <div class="detail">
          <h3 style="margin-top:0">试卷 #${state.selectedPaperId} 分析</h3>
          <p><span class="muted">状态</span> ${escapeHtml(a.status)}</p>
          <p><span class="muted">模型快照</span></p>
          <pre class="block">${escapeHtml(JSON.stringify(a.ai_model_snapshot || {}, null, 2))}</pre>
          <p><span class="muted">摘要</span></p>
          <pre class="block">${escapeHtml(a.summary || '')}</pre>
          <p><span class="muted">薄弱点</span> ${escapeHtml((a.weak_points || []).join('、'))}</p>
          <p><span class="muted">改进计划</span></p>
          <ul>${plan.map((x) => '<li>' + escapeHtml(x) + '</li>').join('')}</ul>
        </div>`;
    } catch (e) {
      pane.innerHTML = `<p class="muted">无法加载详情：` + escapeHtml(e.data && e.data.code ? e.data.code : e.message) + '</p>';
    }
    refreshPapersListOnly(root);
  }

  /** Re-render list selection highlight without full reload */
  function refreshPapersListOnly(root) {
    const pane = root.querySelector('#paperPane .paper-list');
    if (!pane) return;
    pane.querySelectorAll('.paper-item').forEach((el) => {
