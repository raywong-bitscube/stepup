...
**发版 / 升级**: [docs/README.md](../README.md)、[部署与升级说明（20260404#01）](../releases/20260404%2301_DEPLOY_AND_UPGRADE.md)
...
3. `Create` 若有 DB，走 `createDB`：
   ...
   - 插入 `exam_paper`
   - 写入 **`ai_call_log`**（分析适配器轨迹：成功 / 超时 / HTTP 错误 / 回退 mock；见 `ailog.Writer`）
...
- `studentpaper.Service` 在 `createDB` 得到 `paper_id` 后调用 `writeAILog`，经 `ailog.Writer` 插入 **`ai_call_log`**。
- HTTP 适配器（`http_adapter.go`）每次调用返回 `AnalyzeTrace`：耗时、`http_status`、错误阶段与截断信息、`endpoint` 主机、`chat_model`、是否 `fallback_to_mock`。
- 表缺失或写入失败时 **忽略错误**，不影响上传；管理端用 `GET /api/v1/admin/ai-call-logs` 查询。详见 [`ai_model_log_v0.1_260403.md`](./ai_model_log_v0.1_260403.md)。
...
- 审计：`audit_log`
- AI 调用轨迹：`ai_call_log`（与 `paper_analyze` 对齐，不含密钥与完整 body）
